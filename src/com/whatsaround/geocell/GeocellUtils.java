...
import com.whatsaround.geocell.comparator.DoubleTupleComparator;
import com.whatsaround.geocell.model.BoundingBox;
...
public final class GeocellUtils {
	 ...
	 * 
	 * Returns True if the given cells are in the same row (columnTest=False) or
	 * in the same column (columnTest=True).
	 * 
	 * @param cell1
	 *            : The first geocell string.
	 * @param cell2
	 *            : The second geocell string.
	 * @param columnTest
	 *            : A boolean, where False invokes a row collinearity test and 1
	 *            invokes a column collinearity test.
	 * @return A bool indicating whether or not the given cells are collinear in
	 *         the given dimension.
	 */
	public static boolean collinear(String cell1, String cell2,
			boolean columnTest) {
	 ...
	 * 
	 * @param cellNE
	 *            : The Northeast geocell string.
	 * @param cellSW
	 *            : The Southwest geocell string.
	 ...
	 * 
	 * @param cell
	 *            : The geocell string for which to calculate
	 *            adjacent/neighboring cells.
	 * @return A list of 8 geocell strings and/or None values indicating
	 *         adjacent cells.
	 */

	public static List<String> allAdjacents(String cell) {
		List<String> result = new ArrayList<String>();
		for (int[] d : Arrays.asList(NORTHWEST, NORTH, NORTHEAST, EAST,
	 ...
	 * 
	 * @param cell
	 *            : The geocell string whose neighbor is being calculated.
	 * @param dir
	 *            : An (x, y) tuple indicating direction, where x and y can be
	 *            -1, 0, or 1. -1 corresponds to West for x and South for y, and
	 *            1 corresponds to East for x and North for y. Available helper
	 *            constants are NORTH, EAST, SOUTH, WEST, NORTHEAST, NORTHWEST,
	 *            SOUTHEAST, and SOUTHWEST.
	 * @return The geocell adjacent to the given cell in the given direction, or
	 *         None if there is no such cell.
	 */
		...
		while (i >= 0 && (dx != 0 || dy != 0)) {
			...
			if (dx == -1) { // Asking for left.
				if (x == 0) { // At left of parent cell.
					x = GEOCELL_GRID_SIZE - 1; // Becomes right edge of adjacent
												// parent.
				} else {
			...
			if (dy == 1) { // Asking for above.
				...
				if (y == 0) { // At bottom of parent cell.
					y = GEOCELL_GRID_SIZE - 1; // Becomes top edge of adjacent
												// parent.
				} else {
					y--; // Adjacent, same parent.
					dy = 0; // Done with y.
	 ...
	 * 
	 * @param point
	 *            : The geotypes.Point to compute the cell for.
	 * @param resolution
	 *            : An int indicating the resolution of the cell to compute.
	 * @return The geocell string containing the given point, of length
	 *         resolution.
	 */
	public static String compute(Point point, int resolution) {
		...
		while (cell.length() < resolution) {
			...
			int x = Math
					.min((int) (GEOCELL_GRID_SIZE * (point.getLon() - west) / (east - west)),
							GEOCELL_GRID_SIZE - 1);
			int y = Math.min((int) (GEOCELL_GRID_SIZE
					* (point.getLat() - south) / (north - south)),
					GEOCELL_GRID_SIZE - 1);
	 ...
	 * 
	 * @param cell
	 * @return Returns whether or not the given geocell string defines a valid
	 *         geocell.
	 */
	 ...
	 * 
	 * @param char_
	 * @return Returns the (x, y) of the geocell character in the 4x4 alphabet
	 *         grid.
	 */
	 ...
	 * 
	 * @param pos
	 * @return Returns the geocell character in the 4x4 alphabet grid at pos.
	 *         (x, y).
	 */
	public static char subdivChar(int[] pos) {
		// NOTE: This only works for grid size 4.
		return GEOCELL_ALPHABET.charAt((pos[1] & 2) << 2 | (pos[0] & 2) << 1
				| (pos[1] & 1) << 1 | (pos[0] & 1) << 0);
	}
	 ...
	 * 
	 * @param p1
	 *            : indicating the first point.
	 * @param p2
	 *            : indicating the second point.
	 * @return The 2D great-circle distance between the two given points, in
	 *         meters.
	 */
	...
	/**
	 * This function is used to fix issue 10: GeocellUtils.distance(...) uses
	 * Math.acos(arg) method. In some cases arg > 1 (i.e 1.0000000002), so acos
	 * cannot be calculated and the method returns NaN.
	 * 
	 * @param d
	...
	public static Point getLocation(Professional entity) {

		Point location = new Point();

		location.setLat(entity.getLatitude());
		location.setLon(entity.getLongitude());

		return location;
	}
	
	public static Point getCareerLocation(Career entity) {
